...
**Implemented 2026-09-20/21 — phone check run 2026-09-21 (results and reading in [scratchy-audio-phones.md](scratchy-audio-phones.md))** — see [docs/specs/AUDIO_LOAD_BUDGET.md](../specs/AUDIO_LOAD_BUDGET.md) (§8 "As Shipped") and [docs/tasks/AUDIO_LOAD_BUDGET.md](../tasks/AUDIO_LOAD_BUDGET.md); measurements in [docs/PERFORMANCE.md](../PERFORMANCE.md). Shipped as described below, with these outcomes: the **Audio Load** panel (Fleet Params → Transport & Composition, next to Tempo) with Light / Standard / Full presets and a fine slider; robots over the budget **stand by** (first come, first served, solo always sounds); polyphony ceiling, LFO tiers (drift off ≤ Standard, filter-frequency/Q LFOs off on Light, EQ-gain kept, robot LFOs capped 4 / 12 by *measured* cost) and the boot-time latency hint (Light → `playback`), all driven from one dial; default Light on phone-like devices; `?load=` pins it and the choice is mirrored into the URL; held-off LFO and drift controls grey out with a label; `?debug` shows `audible n/12` and the budget line; `npm run perf:audio` measures render capacity. **Desktop, same-session A/B against the pre-feature build:** Light −29 % (`charlie`) / −37 % (`bravo`) peak render capacity, Standard −13 % peak on `bravo`, Full unchanged (±0.03), robot-LFO stress at the caps < 0.9. **One gate missed** — Standard's mean on `bravo` is 0.069 below Full (needed ≥ 0.10); accepted and re-set on 2026-09-21 (spec decision N). **Phone check (2026-09-21, Pixel 8, by ear, one take per cell):** Light was the only preset clean or near-clean on both worlds; Standard and Full clicked or dropped out on `bravo`; whether Standard should take the `playback` latency (decision J) is **not settled** — the phone was charging and the runs were back to back, and four takes of `bravo` Standard + `playback` ranged from out-by-0:30 to clean until 1:15. `bravo` Full also dropped out on a rested, unplugged phone (~15 s, then one still unrecovered after 40 s), each time with the `?debug` overlay showing nothing wrong. **Decided 2026-09-21 (Crawford):** Standard stays on `interactive` pending a rested re-run (decision J); the missed gate is accepted and re-set; no more phone runs for now; the read-only output diagnostic in `?debug` (master and pre-chain level taps, a silent-while-sounding event, non-finite events, and the browser's `playbackStats` underrun counts) is built — [specs/AUDIO_OUTPUT_DIAGNOSTIC.md](../specs/AUDIO_OUTPUT_DIAGNOSTIC.md) — and its phone run is Crawford's. Not pushed.
