...
- **J. Standard's latency hint: `interactive`** (unchanged from today). Only Light selects `playback`. Revisit only if the phone A/B (`load=standard` vs `load=standard&latency=playback`, §5.3 item 6) shows the hint matters at Standard. **The phone A/B ran on 2026-09-21 and was inconclusive** (`charlie` favoured `playback` a little, `bravo` did not; the two comparisons are confounded in opposite directions, and one URL ranged from out-by-0:30 to clean past a minute), so **Crawford chose to defer (2026-09-21): Standard stays `interactive` until a rested, unplugged re-run of Standard vs Standard + `playback` (several takes each, alternating) or a desktop A/B says otherwise.**
