...
4. **Render-capacity gate (desktop, measured).** Using `npm run perf:audio` on pinned worlds `charlie:200:-30` and `bravo:-150:90` (bravo has 5 running global LFOs, so it exercises the LFO tiers), **3 runs each in the foreground, one at a time**, a 4-minute time series per run, no orphaned Chrome before/after: **Light lowers the peak-window render capacity (the highest 15-second bucket mean) versus Full by ≥ 25% on `bravo`** (robots, polyphony, drift and the filter LFOs together) **and by ≥ 15% on `charlie`** (robots and polyphony alone); **Standard lowers `bravo`'s peak window by ≥ 10% and its mean by ≥ 0.10 versus Full** (drift alone was measured at ≈ 0.10 for seven LFOs), with **no `charlie` gate for Standard**. *Revised 2026-09-21 (decision M, §7): the original text gated `charlie` at Light ≥ 25% and Standard ≥ 10%; plan task 11 measured 18.2% and −1.2% with the caps alone, so the anchors stay 4 / 8 and the gates were re-set to what each lever can deliver.* **Full is within ±0.03 of the pre-change baseline** taken in the same session (A/B, per `perf-harness-measurement-hygiene`). The pre-change baseline is recorded *first* (plan Task 3). *Standard's mean gate on `bravo` was missed (0.069 measured against ≥ 0.10) and is re-set to what Standard delivers — decision N, §7.*
...
- **L. A held-off LFO is greyed out** (disabled, values still shown, "Held off by Audio Load" label): yes — chosen over the more editable "note only" alternative I had recommended. **Update 2026-09-22:** the greyed-out accent color was refined from a flat white/black (too harsh against the surrounding surface) to a monochrome slate gray (`.sc-held-off` in `HeldOffNote.css`), while leaving the front face's own surface/text colors alone — see that file's own comment for the "why" and the exact values.

- **M. The anchors stay 4 / 8 / 12 robots, and the render-capacity gates were re-set** (Crawford, 2026-09-21, after plan task 11 — see `docs/PERFORMANCE.md` "Caps-only measurement"). Measured on `charlie` (no LFOs) with the caps alone: Light −18.2% (needed 25%), Standard −1.2% (needed 10%; its 8-robot cap is inert when audible robots peak at 7.8). Rather than lower the anchors to ≈ 3 / 6, the gates now say what each lever can deliver: Light ≥ 15% on `charlie` and ≥ 25% on `bravo`; Standard judged on `bravo` only (peak ≥ 10%, mean ≥ 0.10). Light on `charlie` therefore has a thin margin (measured 18.2%, range 13–22%) and is re-checked in plan task 24.

- **J. Standard's latency hint** — **resolved 2026-09-21: `playback`, replacing `interactive`.** The first phone A/B (by ear) was inconclusive and Crawford deferred; that same day, the output diagnostic's `underruns` count gave a clean same-session, same-world (`bravo`) head-to-head over ~5 minutes: Standard/`interactive` **1227** underruns vs Standard/`playback` **186** — about 6.6× fewer with `playback` (`docs/todo/scratchy-audio-phones.md`, "Standard, and decision J"). The cost is measured too: `playback` gives a repeatable ≈266 ms average output latency on this phone (vs ~22–25 ms under `interactive`), identical across two separate sessions — acceptable for a generative soundscape with no rhythm the user plays along to. Not yet checked: whether `charlie` (the calm world) shows the same gap. **Shipped 2026-09-21 (commit `6e8910c`):** `LOAD_PLAYBACK_BELOW` moved from 0.4 to 0.61 (strictly above Standard's 0.6 anchor, strictly below Full's 1), so Standard now takes `playback` too, on Crawford's confirmation.

- **N. Standard's mean gate on `bravo` is accepted as missed and re-set** (Crawford, 2026-09-21, plan task 24). Measured 0.069 (0.461 → 0.392) against ≥ 0.10; the peak gate (≥ 10 %) was met at 13.2 % but thinly (per-run 6.2–15.7 %). Standard means drift off, up to 8 robots and 12 notes; Light is the preset that relieves a phone. The recorded lever, if a rested phone re-run shows Standard still failing on `bravo`, is taking the filter-frequency/Q LFOs off at Standard (raise `LOAD_FILTER_LFOS_MIN` from 0.4 to just above 0.6; ≈ +0.06 per filter LFO on desktop). It was not taken now because the phone data does not isolate it from Light's 4-robot cap. **The output diagnostic's underrun counts (same day) now bear on this too:** Standard's `bravo` underrun count (1227 over ~5 min, `interactive`) sits far closer to Full (2246) than to Light (9), which is a sharper "Standard is the weak preset" signal than the render-capacity numbers gave; `playback` narrows that gap a lot (186) without closing it. Whether that changes the re-set gate, or argues for the filter-LFO lever after all, is still open.
- **O. No further phone runs for now** (Crawford, 2026-09-21). Only `bravo` Full has been re-run rested and unplugged; the other presets' phone readings still carry the first round's confounders (see `todo/scratchy-audio-phones.md`).
- **P. A read-only master-output diagnostic in `?debug` is approved to build** (Crawford, 2026-09-21): master and pre-chain output level, a finite check, a silent-while-sounding event, and a probe of the browser's playback-stats API — present only under `?debug` — to tell a silent graph from loss after it. Three `bravo` dropouts left every existing overlay reading normal. Specified in [AUDIO_OUTPUT_DIAGNOSTIC.md](AUDIO_OUTPUT_DIAGNOSTIC.md).
- **Q. Standard's robot-LFO cap stays 12** (Crawford, 2026-09-21: "let's see if 12 is acceptable" — to be judged in use). Cap 8 (≈ 0.06 less worst-case load) remains the one-line alternative (`ROBOT_LFO_CAP_STANDARD`). Weighting `pulseWidth` LFOs as several slots is still not adopted.
...
**Still open**
- **Phone check (§5.3 criterion 6)** — run by Crawford on 2026-09-21 and recorded, with a plain reading, in [todo/scratchy-audio-phones.md](../todo/scratchy-audio-phones.md). By ear, one take per cell, on a phone that was charging for runs 3–8: **Light was the only preset clean or near-clean on both worlds** (`bravo` had very light clicks and no full dropout); Standard and Full clicked or dropped out on `bravo`; **decision J (does Standard need `playback`?) is not settled** — it helped a little at Standard on `charlie`, and four takes of `bravo` Standard + `playback` ranged from out-by-0:30 to clean until 1:15; three `bravo` dropouts (Standard at ~4:55, and two on Full on a rested, unplugged phone playing through its speaker — the second still unrecovered after 40 s) left no event in the `?debug` overlay, so the failure is something it does not measure; clicks also occurred in quiet moments, so the always-on graph is a candidate (Phase B) worth re-measuring on the phone. Decided 2026-09-21 (Crawford): decision J stays `interactive` pending a rested re-run (§7 J); the missed gate is accepted and re-set (N); no more phone runs for now (O); the master-output-level diagnostic in `?debug` is approved to build (P).
- **Standard's mean on `bravo`** — resolved 2026-09-21: accepted and re-set (decision N, §7); the filter-LFO lever stays recorded in case a rested phone re-run shows Standard still failing.
- **Cap 12 vs 8 for Standard's robot LFOs** — resolved 2026-09-21: **stays 12** (Crawford: "let's see if 12 is acceptable"; 8 is the one-line alternative if use shows it is not; decision Q, §7). **Whether a `pulseWidth` LFO (≈ 7× the cost) should count as several slots** — still unanswered; shipped as a plain count.
- **Lore copy** ("Standing by", "Held off by Audio Load", the Audio Load panel labels) is first-pass invented text — **accepted as fine for now (Crawford, 2026-09-21)**; the panel layout was checked at 390 px and 1280 px in a real browser but not on a phone.
