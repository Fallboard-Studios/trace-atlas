...
**The phone run (§5.3 criterion 12) — done, 2026-09-21.** Three runs, 17 screenshots, recorded with a plain reading in [todo/scratchy-audio-phones.md](../todo/scratchy-audio-phones.md). `playbackStats` worked on the Pixel's Chrome 153 as expected (never `n/a`). **The diagnostic answered the question it was built for: the clicks are playback underruns** — the browser's own count tracked what Crawford heard and saw as red closely enough to call it settled (2246 underruns on a "always lots of red" `bravo` Full run vs 9 on `bravo` Light, vs 82 on a lightly-clicking `charlie` Full run). No full silent-graph dropout occurred in any of the three runs. **The latency thread is now resolved, not just narrowed.** A same-day follow-up run (`bravo` Standard vs Standard + `playback`, for [Audio Load Budget decision J](AUDIO_LOAD_BUDGET.md#decisions-resolved-2026-09-20-crawford)) showed the identical `lat 266ms (21–309)` reading, to the millisecond, from a second, separate page load — ruling out a stale first-sample artifact. It is a real, repeatable property of the `playback` hint on this phone (vs ~22–25 ms under `interactive`), and it doubled as decisive evidence for decision J: Standard/`interactive` logged 1227 underruns against Standard/`playback`'s 186 over the same ~5 minutes on `bravo`, and Standard now ships with `playback` too.
...
- **The Light latency figure** (266 ms average) is unexplained; a longer Light run would show whether it settles.
- **Whether `bravo` still drops out completely** under some condition (heat, a longer session) is untested by this run — it was three single sessions, no full dropout occurred, and the log only keeps the last 8 events.
- **Not done, deliberately:** no fix for the underruns/dropouts themselves; no change to the Audio Load presets — that decision (and whether the ~200× underrun difference settles it) is Crawford's.
